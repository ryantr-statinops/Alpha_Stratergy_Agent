# Canonical Strategy Patterns

This document shows approved ways to compose Round 2 data, features, operators, masks, and position APIs. It is a construction guide, not a promise that a pattern will pass backtesting. Exact fields and parameters must be preregistered and validated.

## 1. Pattern Selection

| Research thesis | Preferred mode | Canonical pattern |
|---|---|---|
| Persistent quality plus market regime | `time_series` | Quality state -> trend -> tiered long |
| Relative value/quality | `cross_sectional` | Ratio -> eligibility -> rank -> neutral weights |
| New report information | Either | Persistent state + report-event overlay |
| Price participation/liquidity | `cross_sectional` | Primary factor + liquidity eligibility |
| Breakout with fundamental support | `time_series` | Quality state -> causal breakout -> long/exit |

Mode must be selected from the economic question before backtesting. See `mode_contract.md`.
