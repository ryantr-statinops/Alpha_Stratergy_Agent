...
"""
name:    MomFast2_30min
summary: Momentum: ROC(13) strength-2 — 30min
...
class CustomStrategy(SimpleAlgorithm):
    ...
    def __algorithm__(self):
        ...
        long_setup = ((roc_smooth > 0) & (return_roll > 0)) & (adx > self.adx_entry_threshold)
        short_setup = ((roc_smooth < 0) & (return_roll < 0)) & (adx > self.adx_entry_threshold)
        exit_setup = ((self.op.crossed_below(roc_smooth, 0) | self.op.crossed_above(roc_smooth, 0)) | self.op.crossed_below(abs(return_roll), self.return_threshold)) | self.op.crossed_below(adx, self.adx_exit_threshold) | trailing_long_exit | trailing_short_exit
