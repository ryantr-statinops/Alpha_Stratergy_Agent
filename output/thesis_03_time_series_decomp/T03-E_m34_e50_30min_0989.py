...
class CustomStrategy(SimpleAlgorithm):
    ...
    def __algorithm__(self):
        ...
        ema = self.feat.ema(close, timeperiod=50)
        adx_val = self.feat.adx(high, low, close, timeperiod=14)

        long_setup = (close > ema) & structured & (adx_val > 18)
        short_setup = (close < ema) & structured & (adx_val > 18)
        exit_setup = self.op.crossed_below(close, ema) | self.op.crossed_above(close, ema) | chaotic | (adx_val < 12)
