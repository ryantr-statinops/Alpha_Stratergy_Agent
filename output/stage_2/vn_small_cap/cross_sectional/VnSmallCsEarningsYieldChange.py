"""
name:    VnSmallCsEarningsYieldChange
summary: Earnings-yield-change rank tilt on the validated value-trend engine.
idea:    A rising earnings yield (cheapening valuation with stable fundamentals)
         marks a value entry point, distinct from the level earnings yield in
         the core. Positive delta tilts the validated value-trend engine toward
         names getting cheaper.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        ey_change = self.feat.delta_panel(earnings_yield)
        ...
        ey_change_rank = self.op.rank_cs_panel(ey_change, mask=base_eligible)
        tilt = 0.9 + ey_change_rank * 0.2
