"""
name:    VnSmallCsCoreProfitability
summary: Core-profitability rank tilt on the validated value-trend return engine.
idea:    Small caps often inflate profit via financial income. Core profit
         (net profit + financial expense - financial income) measures earnings
         from operations; a higher core-profitability tilt avoids non-operating
         inflation while keeping the validated earnings-yield x trend core.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        fin_income = self.data.fun_is_financial_income_quarterly_panel
        fin_expense = self.data.fun_is_financial_expenses_quarterly_panel

        core_profit = self.feat.safe_divide_panel(
            net_profit + fin_expense - fin_income, total_assets_q
        )
        ...
        base_eligible = (
            ...
            & ((fin_income >= 0) | (fin_income < 0))
            & ((fin_expense >= 0) | (fin_expense < 0))
            & ((core_profit >= 0) | (core_profit < 0))
        )
        ...
        core_profit_rank = self.op.rank_cs_panel(core_profit, mask=base_eligible)
        tilt = 0.9 + core_profit_rank * 0.2
