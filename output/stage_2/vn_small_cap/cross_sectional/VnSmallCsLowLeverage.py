"""
name:    VnSmallCsLowLeverage
summary: Low-leverage gate on the validated value-trend return engine.
idea:    The independent leverage factor acts as a gate on the validated
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        liabilities = self.data.fun_bs_liabilities_annual_panel
        ...
        base_eligible = (
            ...
            & (capital_strength > 0.15) & (liabilities >= 0) & (total_assets > 0)
            & ((leverage >= 0) | (leverage < 0))
        )
        traded_value = self.feat.rolling_value_panel(close, volume)
        liquidity_rank = self.op.rank_cs_panel(traded_value, mask=base_eligible)
        eligible = base_eligible & (liquidity_rank > 0.40) & (leverage < 0.70)
