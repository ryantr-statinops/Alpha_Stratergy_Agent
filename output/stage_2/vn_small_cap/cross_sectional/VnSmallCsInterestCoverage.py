"""
name:    VnSmallCsInterestCoverage
summary: Interest-coverage rank tilt on the validated value-trend return engine.
idea:    Small caps with pre-tax profit well above financial expense are less
         exposed to distress and forced financing. High coverage tilts the
         validated earnings-yield x trend core toward financially resilient names.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        coverage = self.feat.safe_divide_panel(pre_tax, fin_expense)
        ...
        base_eligible = (
            ...
            & (capital_strength > 0.15) & (pre_tax > 0) & (fin_expense > 0)
        )
        ...
        coverage_rank = self.op.rank_cs_panel(coverage, mask=base_eligible)
        tilt = 0.9 + coverage_rank * 0.2
