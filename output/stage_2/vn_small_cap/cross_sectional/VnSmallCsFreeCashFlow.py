"""
name:    VnSmallCsFreeCashFlow
summary: Positive-FCF gate on the validated value-trend return engine.
idea:    The independent free-cash-flow factor acts as a gate on the validated
         value-trend return engine; it does not claim independent PnL. CAPEX is
         retained as a nonpositive outflow gate.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        fcf = self.feat.safe_divide_panel(cfo + capex, total_assets)
        ...
        base_eligible = (
            ...
            & (capital_strength > 0.15) & (capex <= 0) & (total_assets > 0)
            & ((cfo >= 0) | (cfo < 0))
        )
        traded_value = self.feat.rolling_value_panel(close, volume)
        liquidity_rank = self.op.rank_cs_panel(traded_value, mask=base_eligible)
        eligible = base_eligible & (liquidity_rank > 0.40) & (fcf > 0)
