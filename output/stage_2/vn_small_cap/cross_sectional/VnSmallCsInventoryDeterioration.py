"""
name:    VnSmallCsInventoryDeterioration
summary: Inventory-quality gate on the validated value-trend return engine.
idea:    The independent inventory factor acts as a gate on the validated
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        inventory = self.data.fun_bs_inventories_net_annual_panel
        total_assets = self.data.fun_bs_total_assets_annual_panel

        ratio = self.feat.safe_divide_panel(inventory, total_assets)
        deterioration = self.feat.rolling_mean_panel(self.feat.delta_panel(ratio))
        ...
        base_eligible = (
            ...
            & (capital_strength > 0.15) & (inventory >= 0) & (total_assets > 0)
            & ((deterioration >= 0) | (deterioration < 0))
        )
        traded_value = self.feat.rolling_value_panel(close, volume)
        liquidity_rank = self.op.rank_cs_panel(traded_value, mask=base_eligible)
        eligible = base_eligible & (liquidity_rank > 0.40) & (deterioration <= 0)
