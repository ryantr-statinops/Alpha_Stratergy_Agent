"""
name:    VnSmallCsRoaImprovement
summary: ROA-improvement rank tilt on the validated value-trend return engine.
idea:    The independent ROA-improvement factor tilts the validated
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        roa = self.feat.safe_divide_panel(net_profit, total_assets_q)
        roa_improvement = self.feat.rolling_mean_panel(self.feat.delta_panel(roa))
        ...
        base_eligible = (
            ...
            & ((roa_improvement >= 0) | (roa_improvement < 0))
        )
        traded_value = self.feat.rolling_value_panel(close, volume)
        liquidity_rank = self.op.rank_cs_panel(traded_value, mask=base_eligible)
        improvement_rank = self.op.rank_cs_panel(roa_improvement, mask=base_eligible)
