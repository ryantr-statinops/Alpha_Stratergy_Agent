"""
name:    VnSmallCsConservativeAssetGrowth
summary: Conservative-growth gate on the validated value-trend return engine.
idea:    The independent asset-growth factor acts as a gate on the validated
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        asset_change = self.feat.delta_panel(total_assets)
        growth_rate = self.feat.safe_divide_panel(asset_change, total_assets)
        growth = self.feat.rolling_mean_panel(growth_rate)
        ...
        base_eligible = (
            ...
            & (capital_strength > 0.15) & (total_assets > 0)
            & ((growth >= 0) | (growth < 0))
        )
        traded_value = self.feat.rolling_value_panel(close, volume)
        liquidity_rank = self.op.rank_cs_panel(traded_value, mask=base_eligible)
        eligible = base_eligible & (liquidity_rank > 0.40) & (growth < 0.30)
