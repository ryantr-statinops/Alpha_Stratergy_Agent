"""
name:    VnSmallCsFinancialNetPayoutSecuritiesProbe
summary: Purely diagnostic probe (Gate 6 sector decomposition). Ranks the
          persistent net-payout-yield signal ONLY within the securities
          sub-population of the financial mask (margin-deposit intensity
          dominant, after excluding insurers and banks). If the parent
          VnSmallCsFinancialNetPayout test edge is a securities-rally bet
          (brokerages up in 2024), this probe carries most of the parent's
          test Sharpe; if banks carry it, this probe is weak. Diagnostic only,
          not a trading thesis.
idea:    Decomposes the financial population into mutually exclusive sub-sectors
          by economic intensity. Population priority: insurance > bank >
          securities.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        non_bank = (bank < 1)
        securities = non_insurance & non_bank & (margin_intensity > 0.03)
        ...
        eligible = (input_sum == input_sum) & (dividends < 0) & (repurchases <= 0) & (common_shares > 0) & (close > 0) & (market_value > 0) & (securities == True)
