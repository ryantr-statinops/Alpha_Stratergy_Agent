"""
name:    VnSmallCsEpsSurpriseDrift
summary: Quarterly EPS report-update growth tilt on the value-trend engine.
idea:    The EPS report-update factor proxies delayed information diffusion and
         mildly tilts the validated value-trend return engine.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        eps_update = self.feat.safe_divide_panel(self.feat.delta_panel(eps), eps)
        ...
        base_eligible = (
            ...
            & ((eps_update >= 0) | (eps_update < 0))
        )
        traded_value = self.feat.rolling_value_panel(close, volume)
        liquidity_rank = self.op.rank_cs_panel(traded_value, mask=base_eligible)
        eps_update_rank = self.op.rank_cs_panel(eps_update, mask=base_eligible)
        tilt = 0.9 + eps_update_rank * 0.2
