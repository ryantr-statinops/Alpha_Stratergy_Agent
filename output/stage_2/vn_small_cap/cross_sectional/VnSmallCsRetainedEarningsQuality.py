"""
name:    VnSmallCsRetainedEarningsQuality
summary: Retained-earnings rank tilt on the validated value-trend return engine.
idea:    Undistributed earnings accumulated relative to assets signal a solid,
         internally-built book value and past profitability. Higher retained
         earnings tilt the validated earnings-yield x trend core toward firms
         with durable capital.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        retained = self.data.fun_bs_undistributed_earnings_quarterly_panel

        book_quality = self.feat.safe_divide_panel(retained, total_assets_q)
        ...
        base_eligible = (
            ...
            & (capital_strength > 0.15) & (retained > 0)
        )
        ...
        book_rank = self.op.rank_cs_panel(book_quality, mask=base_eligible)
        tilt = 0.9 + book_rank * 0.2
