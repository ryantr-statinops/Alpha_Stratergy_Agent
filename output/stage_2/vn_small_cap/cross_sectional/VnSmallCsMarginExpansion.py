"""
name:    VnSmallCsMarginExpansion
summary: Margin-expansion rank tilt on the validated value-trend return engine.
idea:    Profit growing faster than operating expenses signals operating
         leverage and improving scale economics. Positive margin expansion
         tilts the validated earnings-yield x trend core toward inflection.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        profit_margin = self.feat.safe_divide_panel(net_profit, total_assets_q)
        cost_margin = self.feat.safe_divide_panel(selling + admin, total_assets_q)
        margin_expansion = (
            self.feat.delta_panel(profit_margin)
            - self.feat.delta_panel(cost_margin)
        )
        ...
        base_eligible = (
            ...
            & ((selling >= 0) | (selling < 0))
            & ((admin >= 0) | (admin < 0))
            & ((margin_expansion >= 0) | (margin_expansion < 0))
        )
        ...
        margin_rank = self.op.rank_cs_panel(margin_expansion, mask=base_eligible)
        tilt = 0.9 + margin_rank * 0.2
