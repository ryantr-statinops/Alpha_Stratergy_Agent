"""
name:    VnSmallCsIntangibleBurden
summary: Intangible-burden rank tilt on the validated value-trend return engine.
idea:    Large goodwill and intangible assets relative to equity make a small
         cap's book harder to trust and prone to impairment. Favoring clean
         tangible books tilts the validated earnings-yield x trend core.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        intangible = self.data.fun_bs_intangible_fixed_assets_quarterly_panel

        intangible_burden = self.feat.safe_divide_panel(goodwill + intangible, equity_q)
        ...
        base_eligible = (
            ...
            & ((intangible >= 0) | (intangible < 0))
            & ((intangible_burden >= 0) | (intangible_burden < 0))
        )
        ...
        burden_rank = self.op.rank_cs_panel(intangible_burden, mask=base_eligible)
        clean_tilt = 1.5 - burden_rank
