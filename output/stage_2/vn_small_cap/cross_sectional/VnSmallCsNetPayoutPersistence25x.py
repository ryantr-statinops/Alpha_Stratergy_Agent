"""
name:    VnSmallCsNetPayoutPersistence25x
summary: Allocate across small caps by persistent net payout yield: dividends
         plus repurchases minus share issuance, smoothed over time and scaled
         by market value, with 2.5x gross sizing. Market-neutral cross-sectional
         book.
idea:    Dividends and buybacks are substitutes; subtracting issuance measures
         ...
         separates a durable payout policy from one-off special payouts. A
         unit-gross rank-demeaned book caps absolute return, so this ablation
         scales the weights by 2.5 (position-sizing layer only); the rank order
         and the market-neutral construction are unchanged.
"""
