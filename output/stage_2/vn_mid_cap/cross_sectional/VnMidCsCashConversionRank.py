"""
name:    VnMidCsCashConversionRank
...
idea:    H08 - Earnings backed by cash flow are higher quality. Cash Conversion
         test. Success_alpha lesson: improvement vs. own history combined
         with an uptrend vote.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        conversion_baseline = self.feat.rolling_mean_panel(cash_conversion)
        conversion_improve = cash_conversion - conversion_baseline
        trend = self.feat.safe_divide_panel(close, self.feat.ema_panel(close))
        improve_rank = self.op.rank_cs_panel(conversion_improve, mask=eligible)
