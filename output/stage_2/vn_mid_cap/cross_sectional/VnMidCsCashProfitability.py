"""
name:    VnMidCsCashProfitability
summary: Cash-profitability tilt on a validated MID value-trend return engine.
idea:    Cash return on assets is a bounded rank tilt, not independent PnL;
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        cash_roa = self.feat.safe_divide_panel(cfo, assets)
        ...
        base_eligible = (
            ...
            & (assets > 0)
            & ((cfo >= 0) | (cfo < 0))
            & ((cash_roa >= 0) | (cash_roa < 0))
        )
        traded_value = self.feat.rolling_value_panel(close, volume)
        liquidity_rank = self.op.rank_cs_panel(traded_value, mask=base_eligible)
        factor_rank = self.op.rank_cs_panel(cash_roa, mask=base_eligible)
