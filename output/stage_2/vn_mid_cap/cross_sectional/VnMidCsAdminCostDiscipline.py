"""
name:    VnMidCsAdminCostDiscipline
summary: Buy mid caps whose G&A expense burden falls relative to their own
         baseline while the uptrend holds. Administrative cost-discipline
         improvement vote plus trend vote. Covers pair C+E: lean admin costs
         with a stable effective tax rate as quality confirmation.
idea:    C+E pair #31 - Cost discipline vs tax burden. G&A discipline is a
         direct operating-leverage signal; stable tax confirms earnings are
         real. Scores admin-expense improvement on the standard engine.
"""
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        admin_burden = self.feat.safe_divide_panel(admin, total_assets)
        ...
        lean_admin = -admin_burden
        admin_baseline = self.feat.rolling_mean_panel(lean_admin)
        admin_improve = lean_admin - admin_baseline
        trend = self.feat.safe_divide_panel(close, self.feat.ema_panel(close))
        improve_rank = self.op.rank_cs_panel(admin_improve, mask=eligible)
