"""
name:    VnLargeCsFinancialProbe
summary: Purely diagnostic probe. Ranks the persistent net-payout-yield signal
         ONLY within the financial population (insurers + lenders + brokers).
         If the economic-intensity classifier selects real financial names, the
         simulated book is nonzero and differs from the broad parent; an empty
         cross-section produces ~zero metrics and reveals a classifier bug.
         This probe validates the classifier; it is not a trading thesis.
idea:    Diagnostic only. Uses the same economic-intensity flags as the NonFin
         variants but inverts the mask to keep only financials.
"""
