...
"""
name:    TrendMACMA34x68_30min
summary: MA Cross: MA(34)/MA(68) cross — 30min
thesis:  trend | 30min
...
class CustomStrategy(SimpleAlgorithm):
    ...
    def __algorithm__(self):
        ...
        long_setup = ((self.op.crossed_above(fast_ma, slow_ma)) & (return_roll > 0)) & (adx > self.adx_entry_threshold)
        short_setup = ((self.op.crossed_below(fast_ma, slow_ma)) & (return_roll < 0)) & (adx > self.adx_entry_threshold)
        exit_setup = ((self.op.crossed_below(fast_ma, slow_ma) | self.op.crossed_above(fast_ma, slow_ma)) | self.op.crossed_below(abs(return_roll), self.return_threshold)) | self.op.crossed_below(adx, self.adx_exit_threshold) | trailing_long_exit | trailing_short_exit
