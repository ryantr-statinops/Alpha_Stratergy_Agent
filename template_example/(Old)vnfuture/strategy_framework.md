...
### Đúng
```python
# 15min timeframe — theo parameters.md
sma = self.feat.sma(close, timeperiod=10)        # 1 session = 2.5h
...
### Sai
```python
# window=5 không phù hợp cho 15min (parameters.md quy định window=10)
return_roll = self.feat.rolling_mean(return_1, window=5)
```

---

## 5.11 Exit càng đơn giản càng tốt

Exit_setup chỉ nên chứa **1 điều kiện** duy nhất. Exit phức tạp (nhiều OR conditions) làm thoát sớm, giảm lợi nhuận.

### Đúng
```python
exit_setup = (adx < 18)          # 1 condition — đơn giản, hiệu quả
```

### Sai
```python
exit_setup = (close < sma) | (vn30_close < vn30_sma) | (adx < 18)   # 3 conditions → thoát sớm
```

### Nguyên tắc
- Exit chỉ nên có **1 condition** duy nhất
- Thường dùng `adx < 18` (ADX xuống dưới ngưỡng trending) hoặc `crossed_below(close, sma)`
- Không thêm các điều kiện phụ (volume, VN30, ...) vào exit

---

## 5.12 Entry nên có 3-4 conditions

Entry_setup nên có **3-4 conditions** là sweet spot. Quá nhiều condition làm giảm số lượng tín hiệu, quá ít thì không đủ chọn lọc.

### Đúng (3-4 conditions)
```python
long_setup = (close > sma) & (adx > 22) & (return_roll > 0)
# 3 conditions: price trend + adx trending + return positive
```

### Sai (quá nhiều)
```python
long_setup = (close > sma) & (vn30_close > vn30_sma) & (volume > vol_sma) & (adx > 22) & (return_roll > 0)
# 5 conditions → quá ít tín hiệu
```

### Sai (quá ít)
```python
long_setup = (close > sma) & (adx > 22)
# 2 conditions → không đủ chọn lọc
```

### Nguyên tắc
- **3-4 conditions** là sweet spot
- Mỗi condition nên đến từ 1 nhóm khác nhau (price trend, momentum, volume, cross-market)
- Không copy-paste cùng condition cho cả long và short (vi phạm Rule 5.7)
