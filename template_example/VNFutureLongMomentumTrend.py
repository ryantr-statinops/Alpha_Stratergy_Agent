"""
name:    VNFutureLongMomentumTrend
summary: Trade futures on long-only momentum confirmation with trend
         strength and a rolling price filter.
idea:    The strategy stays long only when trend, momentum, and recent
...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        # Long only when the trend is bullish, strength is acceptable, volume is supportive,
        # and recent rolling momentum is still positive.
        long_setup = (macd > macd_signal) & (adx > 18) & (volume > volume_base) & (return_roll > 0)

        # Exit when trend strength fades or the rolling momentum turns flat/negative.
        exit_setup = self.op.or_(adx < 14, return_roll < 0)

        # Apply exit first so the long signal can override it when conditions are valid.
