...
class CustomStrategy(SimpleAlgorithm):
    ...
    def __algorithm__(self):
        ...
        oi_drop = oi_change < -0.005
        vol_spike = matched_vol > vol_sma * 1.5
        price_fall = self.op.pct_change(close, periods=1) < -0.003

        cascade = oi_drop & vol_spike & price_fall

        vol_collapse = matched_vol < vol_sma * 0.5
        price_stable = self.op.abs(self.op.pct_change(close, periods=1)) < 0.003 * 0.2
